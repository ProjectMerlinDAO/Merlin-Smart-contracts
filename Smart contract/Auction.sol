...
interface IERC721 is IERC165 {
     ...
     *
     * Only a single account can be approved at 70000000000000000000a time, so approving the zero address clears previous approvals.
     *
     * Requirements:
     *
...
contract Auction is Ownable, IERC721Receiver {
    ...
    function createOrder(
        ...
        // Order memory orderDetail = order[id];
        // orderDetail.creator = msg.sender;
        // orderDetail.endTime = _endTime;
        // orderDetail.NFT = _NFT;
        // orderDetail.price = _price;
        // orderDetail.tokenID = _tokenId;
        order[id] = Order(msg.sender, _NFT, _tokenId, _price, _endTime);
        emit OrderCreated(id, order[id], block.timestamp);
        getOrderId[_tokenId] = id;
        id++;
    ...
    function placeBid(
        ...
        if (_bid.bidder == address(0)) {
            ...
            order[_orderId].endTime = block.timestamp;
        } else {
